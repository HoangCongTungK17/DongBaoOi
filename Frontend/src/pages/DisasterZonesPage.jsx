...
import {
  ...
  getAllDisasterZones,
  udpateDisasterZone,
} from "../Redux/DisasterZone/Action.js";
...
function SmartLocationPicker({
  ...
  const mapCenter =
    lat && lng && !isNaN(lat) && !isNaN(lng) ? [lat, lng] : [16.0471, 108.2068];
      ...
      <div className="relative flex-1 min-h-[300px] w-full overflow-hidden rounded-xl border border-slate-700 shadow-inner">
          ...
          {hasValidCoords && (
            <>
              ...
              <Circle
                ...
                pathOptions={circleStyleForDanger(dangerLevel)}
              />
            </>
          )}
        ...
        <div className="absolute bottom-3 right-3 z-[999] bg-slate-900/90 backdrop-blur px-3 py-2 text-xs text-slate-300 rounded-lg border border-slate-700 shadow-xl flex flex-col gap-1">
          <span className="flex items-center gap-2">
            <MapPin className="h-3 w-3 text-red-500" /> Kéo thả marker để chọn
            tâm
          </span>
        </div>
      </div>
...
function DisasterZonesPage() {
  ...
  function handleUpdate() {
    ...
    const updatedZone = {
      ...
      dangerLevel: form.dangerLevel,
      centerLatitude:
        parseFloat(form.centerLatitude) || activeZone.centerLatitude,
      centerLongitude:
        parseFloat(form.centerLongitude) || activeZone.centerLongitude,
  ...
  return (
    <div className="min-h-screen bg-slate-950">
      <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8 space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-slate-100">
                Quản lí khu vực thảm họa
              </h1>
              <p className="mt-1 text-sm text-slate-400">
                Xem và quản lý các khu vực thảm họa
              </p>
            </div>
            <button
              onClick={() => navigate("/")}
        ...
        {/* Zones Grid */}
        {disasterStore?.allZonesError && (
          <p className="text-red-500 text-center">
            Có lỗi xảy ra! Vui lòng làm mới trang
          </p>
        )}
        {disasterStore?.allZonesLoading && (
          <p className="text-center text-slate-400 py-8">
            <Loader2 className="h-6 w-6 animate-spin mx-auto mb-2" />
            Đang lấy tất cả các khu vực đang hoạt động…
          </p>
        )}
        <section>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {paginatedZones.map((z) => (
              <div
                key={z.id}
                className="group rounded-2xl border border-slate-800 bg-slate-900 shadow-lg hover:shadow-xl transition-shadow"
              >
                <div className="p-4">
                  <div
                  ...
                  >
                    ...
                    <span className={dangerBadgeClass(z.dangerLevel)}>
                      {z.dangerLevel}
                    </span>
                  </div>
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-slate-300">
                    <div>
                      ...
                      <div>
                        {z.centerLatitude.toFixed(4)},{" "}
                        {z.centerLongitude.toFixed(4)}
                      </div>
                    </div>
                    <div>
                  ...
                  <div className="mt-3">
                    <MiniZoneMap
                      lat={z.centerLatitude}
                      lng={z.centerLongitude}
                      radiusKm={z.radius}
        ...
        <Modal
          ...
          footer={
            <>
              ...
              >
                {disasterStore?.createZoneLoading ||
                disasterStore?.updateZoneLoading
                  ? "Đang xử lý..."
                  : "Tạo Khu Vực"}
              </button>
            </>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-full">
            ...
            <div className="flex flex-col h-full min-h-[350px]">
              <label className="mb-2 text-sm font-medium text-slate-300 flex items-center gap-2">
                <Crosshair className="h-4 w-4 text-indigo-400" /> Vị trí & Phạm
                vi
              </label>
              <SmartLocationPicker
            ...
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-3 text-white outline-none focus:border-indigo-500 transition-all appearance-none"
                  >
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-3 text-white outline-none focus:border-indigo-500 transition-all appearance-none"
                  >
                    {dangerOptions.filter(Boolean).map((d) => (
              ...
              <p className="text-[10px] text-slate-500 italic">
                *Mẹo: Nhập tọa độ hoặc kéo marker trên bản đồ để cập nhật vị
                trí.
              </p>
            </div>
          </div>
